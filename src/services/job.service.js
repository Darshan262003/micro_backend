...
async function listJobsByEmployer(employerId, query = {}) {
  ...
  if (query.status !== undefined) {
    ...
    if (!allowed.has(String(query.status))) {
      throw badRequestError('status must be one of: posted, filled, completed');
    }
  ...
  const data = items.map((job) => ({
    ...job,
    remainingSlots: Math.max(0, Number(job.requiredWorkers) - Number(job.confirmedCount)),
  }));
...
async function updateJobForEmployer(employerId, jobId, body) {
  ...
  if (job.status !== 'posted') {
    const err = new Error('Job can only be updated while status is posted');
    err.status = 400;
    throw err;
  }
  ...
  if (updates.title !== undefined) {
    job.title = String(updates.title).trim();
  }
  if (updates.description !== undefined) {
    job.description = updates.description != null ? String(updates.description) : '';
  }
  if (updates.location !== undefined) {
    job.location = updates.location != null ? String(updates.location) : '';
  }
  if (updates.scheduledDate !== undefined) {
    job.scheduledDate = new Date(updates.scheduledDate);
  }
  if (updates.paymentAmount !== undefined) {
    job.paymentAmount = Number(updates.paymentAmount);
  }
  if (updates.requiredWorkers !== undefined) {
    job.requiredWorkers = Number(updates.requiredWorkers);
  }
...
async function completeJobForEmployer(employerId, jobId) {
  ...
  if (job.status === 'completed' || job.status === 'cancelled') {
    const err = new Error('Job cannot be marked completed in its current status');
    err.status = 400;
    throw err;
  }
...
async function listPostedJobsForWorker(query = {}) {
  ...
  if (query.status !== undefined && String(query.status) !== 'posted') {
    throw badRequestError('Workers can only query status=posted');
  }

  if (query.location && String(query.location).trim()) {
    filters.location = { $regex: String(query.location).trim(), $options: 'i' };
  }
...
async function getPostedJobByIdForWorker(jobId) {
  const job = await Job.findOne({
    _id: new mongoose.Types.ObjectId(jobId),
    status: 'posted',
    scheduledDate: { $gte: startOfTodayLocal() },
    $expr: { $lt: ['$confirmedCount', '$requiredWorkers'] },
  })
    .lean()
    .exec();
